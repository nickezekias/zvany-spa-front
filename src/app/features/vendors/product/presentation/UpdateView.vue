...
async function onFormSubmit(e: FormSubmitEvent) {
  ...
  if (isFormCorrect) {
    ...
    try {
      ...
      nikkToast.success('features.vendors.products.update.successMessage')
...
<template>
  <div class="nikk-container">
    <AppPageTitle
      title="features.vendors.products.update.title"
      subtitle="features.vendors.products.update.subtitle"
    >
    </AppPageTitle>

    <PrimeCard>
      <template #content>
        <div v-if="pageLoading" class="flex">
          <PrimeSkeleton width="100%" height="24rem" />
        </div>
        <PrimeForm
          v-else
          :initialValues="obj"
