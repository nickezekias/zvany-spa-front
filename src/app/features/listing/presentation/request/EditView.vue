...
onMounted(async () => {
  try {
    ...
    const data = await objStore.getSpaceRequestListing(
...
async function onFormSubmit(e: FormSubmitEvent) {
  ...
  if (isFormCorrect) {
    ...
    try {
      await objStore.updateSpaceRequestListing(payload)
      nikkToast.success('features.listings.requests.edit.successDesc')
...
<template>
  <div class="nikk-container">
    <AppPageTitle
      title="features.listings.requests.edit.title"
      subtitle="features.listings.requests.edit.titleDesc"
    >
    </AppPageTitle>
        ...
        <div>
          ...
          >
            <h3 class="text-lg font-medium col-span-2">
              {{ $t('labels.departureAndDestination') }}
            </h3>
